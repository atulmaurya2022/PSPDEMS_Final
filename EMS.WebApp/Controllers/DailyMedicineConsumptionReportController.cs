...
public class DailyMedicineConsumptionReportController : Controller
{
    ...
    public async Task<IActionResult> Export(DateTime? fromDate, DateTime? toDate)
    {
        try
        {
            ...
            bool isTribeniPlant = (plantInfo?.plant_name?.Equals("Tribeni", StringComparison.OrdinalIgnoreCase) == true)
                               || (plantInfo?.plant_code?.Equals("TRB", StringComparison.OrdinalIgnoreCase) == true)
                               || (plantCode?.Equals("TRB", StringComparison.OrdinalIgnoreCase) == true);
            var roleHeader = isTribeniPlant ? "PHARMACIST" : "COMPOUNDER";
            ...
            csv.AppendLine($"MEDICINE NAME,TOTAL AVAILABLE AT {roleHeader} STOCK,ISSUED QTY,EXPIRED QTY,TOTAL AVAILABLE AT {roleHeader} INVENTORY");
